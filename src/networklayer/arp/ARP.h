...
class INET_API ARP : public cSimpleModule
{
  ...
  protected:
    ...
    bool flag_for_packet_sent_out;

    cGate *nicOutBaseGate; //rvr changed
  ...
  protected:
    ...
    virtual int link_broken(int index);

    // This function is used to determine the switch to which the broken link would be connected.
    virtual IPAddress get_next_hop_address(IPAddress node_address, int index);
};
