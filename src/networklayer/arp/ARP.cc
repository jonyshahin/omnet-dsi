...
void ARP::initialize()
{
    ...
    flag1 = true;
    flag_for_packet_sent_out = false;
...
void ARP::processOutboundPacket(cMessage *msg)
{
    ...
    IPAddress edge_to_fail(20,2,7,0); // While mentioning the node, actually a packet which travels via a particular link would be disconnected
    //rvr changed
    ...
    if (!ie->isBroadcast())
    {
        ...
        EV << "I need to change it here:" << "\n";

        //rvr changed
        nicOutBaseGate = gate("nicOut",ie->getNetworkLayerGateIndex());

        if(nicOutBaseGate->isConnected())
        	send(msg, nicOutBaseGateId + ie->getNetworkLayerGateIndex());
        else
        {
        	EV << "Trying to send a packet along a broken link and hence detour" << "\n";
        	int new_index = link_broken(ie->getNetworkLayerGateIndex());
        	send(msg, nicOutBaseGateId + new_index);
        }


        EV << "The index through which the data is sent is " << ie->getNetworkLayerGateIndex() <<  "\n";
        EV << "The gate thru which it is sent is " << nicOutBaseGate << "\n";
        EV << "The current gate id is " << nicOutBaseGate->getId();
        EV << "Is the gate connected" << nicOutBaseGate->isConnected() << "\n";
//        if(edge_to_fail.equals(this_node_address) && flag1 == true && simTime() > 0.1) //use 0.009 for 1000 packets and 0.1 for 10000 packets
//        {
//        	nicOutBaseGate->disconnect();
//        	flag1 = false;
//        }
...
int ARP::link_broken(int index)
{
	int num_interface = ift->getNumInterfaces() - 2;
	int ip_header_bytes = 20;
	EV << "The number of interfaces are " << num_interface;
	IPAddress this_node_address = rt->getRouterId();
	IPAddress dest_address;
	dest_address = get_next_hop_address(this_node_address, index);
	if(index == num_interface - 1) // If you are at the upper bound of the interface then reduce the index
		index--;
	else if(index == num_interface / 2) // If you are at the lower bound of the interface then increase the index
		index++;
	else if(index == 0)	// Else again I increase the index
		index++;
	else if(index == (num_interface / 2) - 1)
		index--;
	else
		index++;

	//Create a new IPdatagram here and send it to the controller only for the first time when the link
	//fails

	if(flag_for_packet_sent_out == false)
	{
		IPAddress nul(0,0,0,0);
		char packet_name[60];
		strcpy(packet_name,"Link failure ");
		strcat(packet_name,this_node_address.str().c_str());
		IPDatagram *datagram = new IPDatagram(packet_name);
		cPacket *pkt = new cPacket(packet_name);
		datagram->setByteLength(ip_header_bytes);

		datagram->encapsulate(pkt);
		datagram->setDestAddress(dest_address);
		datagram->setSrcAddress(this_node_address);
		datagram->setSrcPort(0);
		datagram->setDestPort(0);
		datagram->setFlowId(0);
		datagram->setPacketId(0);
		datagram->setLink_status(1);
		datagram->setLoad(0);
		datagram->setNum_packets_for_this_path(0);
		//datagram->setDiffServCodePoint((unsigned char)"0");
		datagram->setDontFragment(false);
		datagram->setFragmentOffset(0);
		datagram->setKind(0);
		datagram->setTimeToLive(32);
		datagram->setTransportProtocol(2); // Here the protocol is set to 2 for link failure to be checked at controller
		...
		send(datagram,nicOutBaseGateId + num_interface); //send the datagram to the controller which is connected on the last interface
		flag_for_packet_sent_out = true;
	}
	return index;
}

IPAddress ARP::get_next_hop_address(IPAddress this_node_address, int index)
{
	IPAddress dest_address;
	int num_interface = ift->getNumInterfaces() - 2; // Here it is the two because one of the interface is connected to the controller and the other one to the ip because this is arp.
	int switch_id;
	if(this_node_address.getDByte(1) == 1)
	{
		if(this_node_address.getDByte(2) % 2 != 0)
			switch_id = this_node_address.getDByte(2) - (num_interface * num_interface) / 2 + (num_interface - index - 1);
		else
			switch_id = this_node_address.getDByte(2) - (num_interface * num_interface) / 2 + (num_interface - index - 1) - 1;

		dest_address.set(this_node_address.getDByte(0),2,switch_id,0);
	}
	else if(this_node_address.getDByte(1) == 2)   // These work only for switches with 4 ports
	{
		if(index >= num_interface / 2)
		{
			if(this_node_address.getDByte(2) % 2 != 0)
				switch_id = num_interface - index;
			else
				switch_id = num_interface - index + 2;
			dest_address.set((num_interface + 1) * 10,3,switch_id,0);
		}
		else
		{
			if(this_node_address.getDByte(2) % 2 != 0)
				switch_id = this_node_address.getDByte(2) + (num_interface * num_interface) / 2 + index;
			else
				switch_id = this_node_address.getDByte(2) + (num_interface * num_interface) / 2 + index - 1;
			dest_address.set(this_node_address.getDByte(0),1,switch_id,0);
		}
	}
	return dest_address;
}
