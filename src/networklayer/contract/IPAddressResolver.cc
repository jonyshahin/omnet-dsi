...
IRoutingTable *IPAddressResolver::secRoutingTableOf(cModule *host)
{
    // find IRoutingTable
    cModule *mod = host->getSubmodule("routingTable1");
    if (!mod)
        opp_error("IPAddressResolver: IRoutingTable not found as submodule "
                  " `routingTable' in host/router `%s'", host->getFullPath().c_str());
    return check_and_cast<IRoutingTable *>(mod);
...
IRoutingTable *IPAddressResolver::findSecRoutingTableOf(cModule *host)
{
    cModule *mod = host->getSubmodule("routingTable1");
    return dynamic_cast<IRoutingTable *>(mod);
}
